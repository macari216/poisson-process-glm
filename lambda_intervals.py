...
def update_lambda(lambda_hat, d_lams, all_k_indices):
    for i in range(len(d_lams)):
        ...
        all_indices = jnp.where(valid_mask, all_indices, 0)  # Replace out-of-bounds with index 0 ???
...
spikes_quiet = sio.loadmat('/songbirds_data/c57SpikeTimesQuiet.mat')['c57SpikeTimesQuiet']
...
ts_dict_quiet = {key: nap.Ts(spikes_quiet[key, 0].flatten()) for key in range(n_neurons)}
...
X = nmo.convolve.create_convolutional_predictor(kernels, y).reshape(-1, n_fun * n_neurons)
print(X.shape)
...
last_sp = max([jnp.maximum(spikes_quiet_ex_jax[0].max(), spikes_quiet_ex_jax[1].max()) for i in range(14)])
...
spike_indices = np.array([np.array([np.round(ts / binsize).astype(int) + 1 for ts in spikes_quiet_sorted[i]]) for i in range(n_neurons)], dtype=object)
...
spike_indices = np.concatenate([np.tile(k, len(spikes_quiet_sorted[k])) for k in range(spikes_quiet_sorted.shape[0])]).squeeze()
...
for k in range(n_neurons):
    ...
    for y_k in spikes_k:
        ...
        for t_k, n in zip(S_delta.t, S_delta.d):
            tau = round(y_k - t_k)
            lam_k = lam_k.at[k].add(jnp.log(
                            inverse_link_fun(
                                jnp.dot(basis_taus[tau], w_hat[n,:,k])
                            )
                        )
                    )
...
ws_sec = 0.004
spikes_quiet_sorted = np.array([spikes_quiet_sorted[k].squeeze()[spikes_quiet_sorted[k].squeeze() < 9108] for k in range(195)], dtype=object)
merged_intervals, mapped_spikes = compute_intervals_and_mappings(spikes_quiet_sorted, ws_sec)
np.save("/Users/amedvedeva/Simons Foundation Dropbox/Arina Medvedeva/glm_songbirds/songbirds_data/merged_intervals.npy", np.array(merged_intervals))
...
spk_per_int = np.array([sum(len(k) for k in mapped_spikes[interval]) for interval in merged_intervals])
neur_per_int = np.array([sum(k.size > 0 for k in mapped_spikes[interval]) for interval in merged_intervals])
