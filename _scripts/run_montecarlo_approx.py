...
import jax
from poisson_point_process.monte_carlo_approx import tot_spk_in_window, sum_basis_and_dot
from time import perf_counter

from poisson_point_process import raised_cosine_log_eval, compute_unnormalized_log_likelihood
import os
...
max_window = tot_spk_in_window(jnp.array([-history_window,0]), spike_times_neuron, spike_times_concat)
max_window = int(max_window)
print(f"max window: {max_window}")

delta_idx = jax.nn.relu(max_window - neu_spk_idx[0])
shifted_idx = neu_spk_idx + delta_idx
tot_spikes_new = np.hstack((jnp.full(delta_idx, -history_window - 1), spike_times_concat))
...
basis_fn = lambda delta_ts: raised_cosine_log_eval(delta_ts, history_window, n_basis_funcs, width=2, time_scaling=50)
...
for idx in shifted_idx:
    ...
    lam_y += jnp.log(jax.nn.softplus(sum_basis_and_dot(weights[w_idxs], dts, basis_fn)+bias))

########
t0 = perf_counter()
out = compute_unnormalized_log_likelihood(
    weights,
    bias,
    spike_times_concat,
    spike_id_concat,
...
# mc_window = tot_spk_in_window(jnp.array([-history_window,0]), tau_m, spike_times_concat)
