...
def phi_product_int(delta_idx, x, ws, basis_fn):
...
def precompute_Phi_x(x, ws, basis_fn):
    """precompute M_ts_ts' for all possible deltas"""
    M_x = []

    for delta_x in range(ws):
        M_x.append(phi_product_int(delta_x, x, ws, basis_fn))
    ...
    def compute_Mn_half(pair):
        def valid_idx(M_scan, idx):
            spk_in_window = slice_array(
                tot_spikes, idx[1].astype(int), max_window
            )
        ...
        post_idx_array, padding = reshape_for_vmap(jnp.vstack((tot_spikes[0, post_idx], post_idx)), n_batches_scan)
...
def run_scan_fn_k(
    ...
    def scan_k(k_sum, i):
        spk_in_window = slice_array(
            tot_spikes, i[1].astype(int), max_window
        )
    ...
    target_idx_array, padding = reshape_for_vmap(target_spikes, n_batches_scan)
...
def compute_weights(suff,interval,f=jnp.exp,Cinv=None):
    """
    ...
    w_paglm = jnp.linalg.solve(2*a2*suff[1] + Cinv, b)
    return w_paglm
